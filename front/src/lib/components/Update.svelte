...
<script>
    ...
    async function send(){
        ...
        if( !check ){ return }
        const res = await fetch(`http://127.0.0.1:8000/updateAnimals/${inputid}`, {
            method: 'PUT',
        ...
        <div class="flex flex-col lg:flex-row gap-2">
            <div class="w-full flex flex-col gap-2">
                <div class="flex gap-4">
                    ...
                    <input bind:value={inputid} class="w-full duration-200 bg-transparent border border-white/10 rounded-md p-2" type="number" min="0" placeholder="ID">
                </div>
                <div class="flex gap-4">
